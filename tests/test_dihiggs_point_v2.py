...
HEADER = (
    ...
    "width_hh_GeV,total_width_GeV,br_bb,br_cc,br_tautau,br_WW,br_ZZ,br_gammagamma,br_Zgamma,br_gg,"
...
def test_pilot_anchors_at_125_13(tmp_path, name, kwargs, theory_ok):
    ...
    if name == "L06":
        assert 1e-18 < float(row["total_width_GeV"]) < 1e-16
        assert float(row["br_gammagamma"]) > 0.6
        assert float(row["ctau_mm"]) > 10_000
