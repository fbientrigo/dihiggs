...
def test_m2_command_matches_real_named_cli():
    ...
    command = engine.build_command(
        ...
        make_grid(mphi_min=125.20, mphi_max=500.0, n_mphi=7,
    ...
    assert pairs(command) == {
        "--campaign-id": "campaign-17", "--run-id": "run-4", "--mh": "125.2",
        "--mH-min": "125.2", "--mH-max": "500", "--n-mH": "7",
...
def test_m2_engine_metadata_and_columns_are_canonical():
    ...
    assert metadata["mass_convention"]["mh_GeV"] == 125.20
