...
# B. Validation status after Phase 7
...
| exact physical `phi H+H-` coupling | **NOT VERIFIED** |
| `X=lambda6 tan beta` interpretation | **DEFERRED** |
...
\boxed{\text{PHASE 7 PASS}}.
\]

The next derivation may use the verified project `Z6,Z7` definitions, but no trilinear sign may be imported from 2HDMC before its local Feynman-rule convention is translated against the project potential.
